...
void efficiency_raw_producer::Loop()
{
  ...
  // monitoring
  TH1F *true_reco = (TH1F*)(histo_num[get_name_histo_num(0,"costhetastar")]->Clone("reco"));
  TH1F *true_gen = (TH1F*)(histo_num[get_name_histo_num(0,"costhetastar")]->Clone("gen"));
  ...
  assert (h_zuug);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	photrail_pt*=rand->Gaus(1,Smearing(photrail_SCeta,photrail_r9));
	  ...
	  if (is_overflow) reco_has_matched_gen_no_acceptance=false;
	  ...
	  if (reco_has_matched_gen_no_acceptance) histo_num[get_name_histo_num(event_ok_for_dataset_local,*diffvariable)]->Fill(value_diffvariableGEN,weight);
	  ...
	  if (reco_has_matched_gen_no_acceptance && gen_in_acc) {
	    response[get_name_response(event_ok_for_dataset_local,*diffvariable)]->Fill(value_diffvariable,value_diffvariableGEN,weight);
	    response2D[get_name_response(event_ok_for_dataset_local,*diffvariable)]->Fill(value_diffvariable,value_diffvariableGEN,weight);
	  }

	  // monitoring
	  if (reco_has_matched_gen_no_acceptance && event_ok_for_dataset_local==0 && (*diffvariable==TString("costhetastar"))) true_reco->Fill(value_diffvariable,weight);
	  if (gen_in_acc && event_ok_for_dataset_local==0 &&(*diffvariable==TString("costhetastar"))) true_gen->Fill(value_diffvariableGEN,weight);
   ...
   TCanvas *cbla = new TCanvas();
   cbla->cd();

   TFile *raw_xsec_file = new TFile("histo_xsec_costhetastar_EBEB.root");
   TH1F *raw_ngg;
   raw_xsec_file->GetObject("xsec_ngammagammayield",raw_ngg);


   // unf3: unf mio + eff mia
   // unf4: unf mio
   // unf5: unf tao + eff tao
   // unf6: unf tao
   // unf7: effunf mio (NUOVO)

   RooUnfoldBayes unf3(response[get_name_response(0,"costhetastar")],raw_ngg);
   TH1D *u3 = (TH1D*)(unf3.Hreco());
   TH1D *u4 = (TH1D*)(u3->Clone("u4"));
   u3->Divide(histo_eff[get_name_histo_eff(0,"costhetastar")]);
   u3->SetMarkerColor(kBlue);
   u3->SetMarkerStyle(20);
   u3->Draw("P");

   RooUnfoldBayes unf4(response[get_name_response(0,"costhetastar")],raw_ngg);   
   u4->SetMarkerColor(kRed);
   u4->SetMarkerStyle(20);
   u4->Draw("same P");

   TFile *file_tao_unf = new TFile("Unfolding_SysErr.root");
   TH1F *unfolded_tao;
   file_tao_unf->GetObject("Unfolding_Nevt_costt_EBEB",unfolded_tao);
   TFile *file_tao_eff = new TFile("Efficiency_WithSysErr.root");
   TH1F *efficiency_tao;
   file_tao_eff->GetObject("h_costhetastar_EBEB_WithTotErr",efficiency_tao);

   TH1F *u5 = (TH1F*)(unfolded_tao->Clone("u5"));
   u5->Divide(efficiency_tao);
   u5->SetMarkerColor(kBlue);
   u5->SetMarkerStyle(21);
   u5->Draw("same P");
   
   TH1F *u6 = (TH1F*)(unfolded_tao->Clone("u6"));
   u6->SetMarkerColor(kRed);
   u6->SetMarkerStyle(21);
   u6->Draw("same P");

   RooUnfoldResponse *respmatr7 = new RooUnfoldResponse(true_reco,true_gen,response2D[get_name_response(0,"costhetastar")]);
   RooUnfoldBayes unf7(respmatr7,raw_ngg);
   TH1D *u7 = (TH1D*)(unf7.Hreco());
   u7->SetMarkerColor(kGreen);
   u7->SetMarkerStyle(20);
   u7->Draw("same P");


   cout << u6->Integral() << " " << u4->Integral() << " *** " << u4->Integral()/u6->Integral() << endl;
   cout << u5->Integral() << " " << u3->Integral() << " " << u7->Integral() << " *** " << u3->Integral()/u5->Integral() << " " << u7->Integral()/u5->Integral() << endl;


//   raw_ngg->Print();
//   cout << raw_ngg->Integral() << endl;
//   u4->Print();
//   cout << u4->Integral() << endl;
//
//   for (int i=0; i<raw_ngg->GetNbinsX(); i++){
//     cout << raw_ngg->GetBinContent(i+1) << " " << u4->GetBinContent(i+1) << endl;
//   }
